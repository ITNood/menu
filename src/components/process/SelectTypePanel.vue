<template>
  <el-dialog title="选择流程" :visible.sync="dialogVisible" width="90%" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" :before-close="handleClose" append-to-body>
    ...
    <el-row>
      <el-col :span="20">
        <el-row v-if="selectTypes && selectTypes.Status && selectTypes.Status === 'Active'">
          <el-col :span="4" v-for="(name, index) in Object.keys(selectTypes['Elements'])" :key="index" :offset="index%4 > 0 ? 2 : 1">
            <transition name="el-fade-in">
              <el-card :shadow="(selectTypes['Elements'][name]['Status'] && selectTypes['Elements'][name]['Status'] === 'Active')?'hover':'never'">
                <div v-on:click="choiceTypeClick(name,selectTypes['Elements'][name])">
                  ...
                  <div style="padding: 14px;text-align: center; ">
                    <span>{{ name }}</span>
                    <span>{{selectTypes['Elements'][name]['Status']&&selectTypes['Elements'][name]['Status']==='Active'?'(可用)':'(不可用)'}}</span>
                  </div>
                </div>
              </el-card>
            </transition>
