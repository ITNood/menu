        ...
        <el-collapse v-model="activeNames" @change="handleChange">
          <el-collapse-item title="基础信息" name="0">
            ...
            <el-form-item label="名称：" prop="name">
              <el-input :value="businessObject.get('name')" size="mini"
          ...
          <el-collapse-item title="流转信息" name="a">
            <div  v-if="is(businessObject,'bpmn:Gateway')">
              <el-form-item label = "数据：">
                <el-select :value="gatewayCondition[1]" size="mini" v-on:input="(a)=>onChangeField(a + gatewayCondition[2] + gatewayCondition[3],['name'])">
                  <el-option v-for="item in items" :key="item" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="条件：" prop="condition">
                <el-select :value="gatewayCondition[2]" size="mini"  v-on:input="(a)=>onChangeField(gatewayCondition[1] + a + gatewayCondition[3],['name'])">
              ...
              <el-form-item label="阈值：" prop="threshold">
                <el-input-number :value="gatewayCondition[3]" size="mini" v-on:change="(a)=>onChangeField(gatewayCondition[1] + gatewayCondition[2] + a,['name'])"></el-input-number>
              </el-form-item>
<!--              <el-button icon="el-icon-delete" type="danger" round size="mini" v-if="businessObject.outgoing">对换条件</el-button>-->
          ...
          <el-collapse-item title="Decision" name="2" v-if="is(businessObject,'bpmn:Gateway')">
          ...
          <el-collapse-item title="引用信息" name="4"  v-if="is(businessObject,'refBpmn:RefServiceTask')">
              <el-form-item label="引用类型：" prop="type">
                <el-select :value="thisElements[selectIndex].index" size="mini">
                  <el-option :value="item.id" :key="index" :label="item.moduleName" v-for="(item,index) in panelTypes.childNode" disabled></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="引用流程：" prop="ID">
                <el-input :value="businessObject.get('ref')?businessObject.get('ref'):'暂无'" disabled size="mini"></el-input>
              </el-form-item>
            <el-row :gutter="20">
              <el-col :span="12">
              ...
              <el-col :span="12">
                <el-button icon="el-icon-delete" type="danger" round size="mini" v-on:click="()=>onChangeField(null ,['ref'])">清除引用</el-button>
              </el-col>
            </el-row>
          </el-collapse-item>
        </el-collapse>
  ...
  methods: {
    ...
    flushElements(newValue) {
      ...
      if (newValue.length) {
        ...
        this.gatewayCondition = (this.businessObject.name ?? "").replaceAll(/\s*/g, "").match(/^(.*?)([<>=!]+)(.*?)$/) ?? ["","","<",""];
      }
    },
    // 字段更新
